...
import nl.trydev07.bettersmp.api.utils.ReflectionUtil;
...
public class Scoreboard {
    ...
    static {
        try {
            if (ReflectionUtil.nmsOptionalClass("ScoreboardServer$Action").isPresent()) {
                VERSION_TYPE = VersionType.V1_13;
            } else if (ReflectionUtil.nmsOptionalClass("IScoreboardCriteria$EnumScoreboardHealthDisplay").isPresent()) {
                VERSION_TYPE = VersionType.V1_8;
            } else {
                VERSION_TYPE = VersionType.V1_7;
            }

            Class<?> craftChatMessageClass = ReflectionUtil.obcClass("util.CraftChatMessage");
            Class<?> entityPlayerClass = ReflectionUtil.nmsClass("EntityPlayer");
            Class<?> playerConnectionClass = ReflectionUtil.nmsClass("PlayerConnection");
            Class<?> craftPlayerClass = ReflectionUtil.obcClass("entity.CraftPlayer");

            MESSAGE_FROM_STRING = craftChatMessageClass.getDeclaredMethod("fromString", String.class);
            CHAT_COMPONENT_CLASS = ReflectionUtil.nmsClass("IChatBaseComponent");
            ...
            SEND_PACKET = playerConnectionClass.getDeclaredMethod("sendPacket", ReflectionUtil.nmsClass("Packet"));

            PACKET_SB_OBJ = ReflectionUtil.nmsClass("PacketPlayOutScoreboardObjective").getConstructor();
            PACKET_SB_DISPLAY_OBJ = ReflectionUtil.nmsClass("PacketPlayOutScoreboardDisplayObjective").getConstructor();
            PACKET_SB_SCORE = ReflectionUtil.nmsClass("PacketPlayOutScoreboardScore").getConstructor();
            PACKET_SB_TEAM = ReflectionUtil.nmsClass("PacketPlayOutScoreboardTeam").getConstructor();

            if (VersionType.V1_8.isHigherOrEqual()) {
                ENUM_SB_HEALTH_DISPLAY = ReflectionUtil.nmsClass("IScoreboardCriteria$EnumScoreboardHealthDisplay");

                if (VersionType.V1_13.isHigherOrEqual()) {
                    ENUM_SB_ACTION = ReflectionUtil.nmsClass("ScoreboardServer$Action");
                } else {
                    ENUM_SB_ACTION = ReflectionUtil.nmsClass("PacketPlayOutScoreboardScore$EnumScoreboardAction");
                }

                ENUM_SB_HEALTH_DISPLAY_INTEGER = ReflectionUtil.enumValueOf(ENUM_SB_HEALTH_DISPLAY, "INTEGER");
                ENUM_SB_ACTION_CHANGE = ReflectionUtil.enumValueOf(ENUM_SB_ACTION, "CHANGE");
                ENUM_SB_ACTION_REMOVE = ReflectionUtil.enumValueOf(ENUM_SB_ACTION, "REMOVE");
            } else {
    ...
    public void updateLines(String... lines) {
        updateLines(TextUtil.coloredText(lines));
    }
